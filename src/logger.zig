...
fn logWrite(context: LogContext, bytes: []const u8) LogError!usize {
    ...
    try @call(.never_inline, logWriteImpl, .{bytes});
...
pub inline fn log(comptime fmt: []const u8, args: anytype) LogError!void {
    if (stdio_enabled) try logger.print(fmt, args);
}
...
comptime {
    if (@hasDecl(c, "LIB_PICO_STDIO_USB") and c.LIB_PICO_STDIO_USB == 1)
        @compileError("stdio via USB is not supported; use the UART instead.");
}
